...
var elem = document.getElementById('lin')
var hammertime = new Hammer(elem, {});
hammertime.get('pinch').set({
    enable: true
});
var scale = 1
hammertime.on('pinchstart pinchin pinchout', throttle(function (event) {
    console.log(event)
    event.deltaY = scale - event.scale
    scale = event.scale
    event.preventDefault();
    if (event.deltaY < 0) {
        zoom_maker('zoomin', 30);
    } else {
        zoom_maker('zoomout', 30);
    }
}, 100))
